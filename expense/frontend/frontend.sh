...
if [ ! -d "/usr/share/nginx/html/.git" ]; then
    cd /usr/share/nginx/html && git clone https://github.com/Niharika2211/expense-frontend.git . &>>"$LOG_FILE"
...
location /api/ {
    proxy_pass http://prod-expense-internal.nirvanan.online/;  # Ensure trailing slash
}
